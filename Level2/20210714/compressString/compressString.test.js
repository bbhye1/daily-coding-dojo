...
function compress2(s, unit) {
  const compressed = splitWith(s, unit)
    .reduce((acc, current) => {
  ...
  return compressed.filter((a) => a !== 1).join('');
}
...
test('compress', () => {
  [compress1, compress2].forEach((compress) => {

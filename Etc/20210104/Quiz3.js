const q3 = (m, timelines) => {
  let answer = 0;

  let lt = 1;
  let rt = timelines.reduce((acc, cur) => acc + cur);

  while (lt < rt) {
    let sum = Math.floor((lt + rt) / 2);
    let count = 0;
    let rest = sum;

    for (let i = 0; i < timelines.length; i++) {
      if (rest - timelines[i] >= timelines[i + 1]) {
        rest -= timelines[i];
      } else {
        rest -= timelines[i];
        count += 1;
    ...
    if (sum >= Math.max(...timelines) && count <= m) {
      answer = sum;
      rt -= 1;
    } else {
      lt += 1;
